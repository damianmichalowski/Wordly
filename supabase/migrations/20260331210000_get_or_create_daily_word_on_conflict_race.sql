-- Równoległe wywołania get_or_create_daily_word (np. po save settings: refresh + widget)
-- mogły oba przejść SELECT „brak wiersza” i oba zrobić INSERT → 23505 duplicate key.
-- INSERT ... ON CONFLICT DO NOTHING + ponowny SELECT usuwa wyścig.
...
begin
  ...
  on conflict on constraint user_daily_word_unique_day do nothing
  returning id into v_existing_daily_word_id;

  if v_existing_daily_word_id is null then
    select udw.id, udw.word_id
    into v_existing_daily_word_id, v_candidate_word_id
    from public.user_daily_word as udw
    where udw.user_id = v_user_id
      and udw.day_date = v_today
    limit 1;
  ...
  if v_existing_daily_word_id is null then
    raise exception 'user_daily_word row missing after insert race';
  end if;
