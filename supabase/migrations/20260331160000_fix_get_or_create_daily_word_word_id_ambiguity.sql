-- Fix: prevent any possible ambiguous `word_id` references by always qualifying.
-- This migration re-defines the RPC to ensure the deployed function matches the codebase expectations.
...
begin
  ...
  select
    udw.id as daily_word_id,
    udw.word_id as word_id
  ...
  insert into public.user_daily_word as udw (
    user_id,
    day_date,
    word_id
  )
  ...
  returning udw.id into v_existing_daily_word_id;

  insert into public.user_word_progress as uwp (
    user_id,
    word_id,
    status,
    created_at,
